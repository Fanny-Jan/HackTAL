...
//                $path = $this->get('kernel')->getRootDir() . $dossier . $file;
                $file = '79.json';
                $donnees = file_get_contents($dossier . $file);
                $savedData = json_decode($donnees, true);
                $clean = new Regex();
                $prep = [
                    ' a ',
                    ' dans ',
...
//                        }
                ...
                $average = ($totaux[1] + $totaux[2] + $totaux[3] + $totaux[4] + $totaux[5] + $totaux[6]) / 6;

                $nbNeg = count($comNeg);
                $ratioCom = ($nbNeg / $nbCom);
                $json = new json();
//        $json->createJson($totaux, $recap, $nom);


//        $select_values[] = '';
                $json->createJson($totaux, $recap, $average, $file, $donnees);
                $totaux = $recap = $average = '';
                die($file);
            }
        }
        $dir = closedir($dossier);
